...
#######################
cd
if [[ -d "~/pi-top-4-.NET" ]]; then
  cd ~/pi-top-4-.NET
  git pull
else
  git clone https://github.com/pi-top/pi-top-4-.NET.git
  cd ~/pi-top-4-.NET
fi
echo ""
...
################################
~/pi-top-4-.NET/tools/install-global-tool.sh
echo ""

##########################
### Install PowerShell ###
##########################
~/pi-top-4-.NET/tools/install-powershell.sh
echo ""
...
sudo unzip -d /usr/local/lib/ ~/pi-top-4-.NET/libs/opencv-dotnet-4.5.0.zip
...
sudo unzip -d /usr/local/lib/ ~/pi-top-4-.NET/libs/onnxruntime-1.5.2.zip
...
### Finish ###
##############
echo "pi-top .NET Examples are installed."
